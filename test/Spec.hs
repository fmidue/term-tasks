...
main = hspec $ do
  ...
  prop "check oneValidTerm (signature1)" $
    forAll (oneValidTerm (validTerms signature1 (Just "u") 1 10)) (\x -> isJust x ==> isValid signature1 (fromJust x))
  prop "check oneValidTerm (signature2)" $
    forAll (oneValidTerm (validTerms signature2 (Just "e") 1 6)) (\x -> isJust x ==> isValid signature2 (fromJust x))
  prop "check oneValidTerm (signature3)" $
    forAll (oneValidTerm (validTerms signature3 (Just "f") 3 8)) (\x -> isJust x ==> isValid signature3 (fromJust x))
  prop "check oneValidTerm (signature4)" $
    forAll (oneValidTerm (validTerms signature4 Nothing 3 8)) (\x -> isJust x ==> isValid signature4 (fromJust x))
  prop "check oneValidTerm (signature5)" $
    forAll (oneValidTerm (validTerms signature5 (Just "b") 1 8)) (\x -> isJust x ==> isValid signature5 (fromJust x))
  prop "check oneValidTerm (signature6)" $
    forAll (oneValidTerm (validTerms signature6 (Just "a") 1 4)) (\x -> isJust x ==> isValid signature6 (fromJust x))
